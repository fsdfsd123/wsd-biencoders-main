...
def process_encoder_outputs(output, mask, as_tensor=False):
	...
	position = -1
	avg_arr = []
	#print('torch.split(output, 1, dim=0):', len(torch.split(output, 1, dim=0)))
	...
	with open(text_path, 'r', encoding="utf8") as f:
		for line in f:
			...
			if line == '</sentence>':
				...
				sense_inst = -1
				sense_label = -1
				if line.startswith('<instance') and pos == 'NOUN':
...
def load_data_signal_sens(datapath, name, sense):
	text_path = os.path.join(datapath, '{}.data.xml'.format(name))
	gold_path = os.path.join(datapath, '{}.gold.key.txt'.format(name))
	...
	s = []
	sense_right = 0
	sentence_sense_right = 0
	with open(text_path, 'r', encoding="utf8") as f:
		for line in f:
			line = line.strip()
			if line == '</sentence>':

				if sentence_sense_right:
					#print(len(s))
					sentences.append(s)
				sentence_sense_right = 0
				s = []
				...
				lemma = re.sub('&apos;', '\'', lemma)
				if lemma == sense:
					sentence_sense_right = 1
					sense_right = 1
				sense_inst = -1
				sense_label = -1
				if line.startswith('<instance') and pos == 'NOUN' and sense_right:
					sense_inst = re.search('instance id="(.+?)"', line).group(1)
					# annotate sense instance with gold label
					sense_label = gold_labels[sense_inst]
				s.append((word, lemma, pos, sense_inst, sense_label))
